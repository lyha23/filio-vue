...
export default defineComponent({
  ...
  emits: ['mousedown', 'mousemove', 'mouseup', 'click', 'dblclick'],
  data() {
    const result = {
...
<template>
  <div ref="container" class="relative relative m-0 h-full w-full border-0 p-0">
    <slot v-if="progress.isComplete === false" name="progress-bar" :progress="progress">
      <div class="bg-[rgba(0, 0, 0, 0.04)] absolute z-2 h-2 w-full" style="">
        <div
          :style="{
    ...
    <div class="absolute top-0 border-t-3 p-3 text-sky-600">
      ...
      <p>
        尺寸:{{ (info?.size?.x / 1).toFixed(2) }} &times; {{ (info?.size?.y / 1).toFixed(2) }} &times;
        {{ (info?.size?.z / 1).toFixed(2) }} cm
      </p>
    </div>
  </div>
</template>
