...
import { type IDesktop, isSmallScreen } from '@/constants';
import {
  Branch,
  type BranchNode,
  type CheckpointNode,
...
const drawLine = (timelineNode: LinkedCheckpointNode, y: number, i: number, isDiverged: boolean) => {
...
const addLineSvgAnimation = (timeline: GSAPTimeline, duration: number, index: number): GSAPTimeline => {
...
const addDivergingBranchLineAnimation = (timeline: GSAPTimeline, duration: number, index: number): GSAPTimeline => {
  timeline
...
const addConvergingBranchLineAnimation = (timeline: GSAPTimeline, duration: number, index: number): GSAPTimeline => {
  timeline
    .from(svgContainer.value!.querySelector(`.line-${index + 1}`), { scaleY: 0, duration }, `start+=${duration * index}`)
    .from(
      svgContainer.value!.querySelector(`.branch-line-${index + 1}`),
...
const setTimelineSvg = (svgContainer: Ref<HTMLElement | undefined>, timelineSvg: Ref<SVGSVGElement | undefined>) => {
...
// , [timelineSvg, svgContainer, svgWidth, rightBranchX, screenContainer, props.isDesktop, svgLength]);
...
<template>
  <section :id="MENULINKS[3].ref" class="section-container relative min-h-screen w-full flex flex-col select-none justify-center py-8">
    <div class="flex flex-col">
      ...
      <h2 class="seq mt-2 w-full text-2xl md:max-w-2xl">A quick recap of proud moments</h2>
    </div>
